...
func NotifyDatasetRegistryRecommend(optUser *models.User, dataset *models.DatasetRegistry) {
	for _, notifier := range notifiers {
		notifier.NotifyDatasetRegistryRecommend(optUser, dataset)
	}
}

func NotifyAimodelRecommend(optUser *models.User, aimodel *models.AiModelManage) {
	for _, notifier := range notifiers {
		notifier.NotifyAimodelRecommend(optUser, aimodel)
	}
}

func NotifyCreateDataset(doer *models.User, u *models.User, repo *models.DatasetRegistry) {
	for _, notifier := range notifiers {
		notifier.NotifyCreateDataset(doer, u, repo)
	}
}
func NotifyDeleteDataset(doer *models.User, u *models.User, repo *models.DatasetRegistry) {
	for _, notifier := range notifiers {
		notifier.NotifyDeleteDataset(doer, u, repo)
	}
}

func NotifyCreateAimodel(doer *models.User, u *models.User, aimodel *models.AiModelManage) {
	for _, notifier := range notifiers {
		notifier.NotifyCreateAimodel(doer, u, aimodel)
	}
}

func NotifyDeleteAimodel(doer *models.User, u *models.User, aimodel *models.AiModelManage) {
	for _, notifier := range notifiers {
		notifier.NotifyDeleteAimodel(doer, u, aimodel)
	}
}
...
func NotifyAlmostEndingAIDebugTask(cloudbrain *models.Cloudbrain, duration time.Duration) {
	for _, notifier := range notifiers {
		notifier.NotifyAlmostEndingAIDebugTask(cloudbrain, duration)
	}
}
