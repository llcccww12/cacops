...
func (t *actionNotifier) NotifyDatasetRegistryRecommend(optUser *models.User, dataset *models.DatasetRegistry) {
	...
	act := &models.Action{
		OpType:    models.ActionDatasetRecommended,
		ActUserID: dataset.OwnerID,
		DatasetID: datasetId,
		IsPrivate: dataset.IsPrivate,
		Content:   fmt.Sprintf("%s|%s", dataset.ID, dataset.DisplayName()),
	}
...
func (*actionNotifier) NotifyCreateDataset(doer *models.User, u *models.User, dataset *models.DatasetRegistry) {
	var datasetId *string
	if dataset.ID != "" {
		datasetId = &dataset.ID
	}
	if err := models.NotifyWatchers(&models.Action{
		ActUserID: doer.ID,
		ActUser:   doer,
		OpType:    models.ActionCreateDataset,
		DatasetID: datasetId,
		Dataset:   dataset,
		IsPrivate: dataset.IsPrivate,
		Content:   fmt.Sprintf("%s|%s", dataset.ID, dataset.DisplayName()),
	}); err != nil {
		log.Error("notify watchers(dataset) '%d/%d': %v", doer.ID, dataset.ID, err)
	}
}
func (*actionNotifier) NotifyDeleteDataset(doer *models.User, u *models.User, dataset *models.DatasetRegistry) {
	var datasetId *string
	if dataset.ID != "" {
		datasetId = &dataset.ID
	}
	if err := models.NotifyWatchers(&models.Action{
		ActUserID: doer.ID,
		ActUser:   doer,
		OpType:    models.ActionDeleteDataset,
		DatasetID: datasetId,
		Dataset:   dataset,
		IsPrivate: dataset.IsPrivate,
		Content:   fmt.Sprintf("%s|%s", dataset.ID, dataset.DisplayName()),
...
func (*actionNotifier) NotifyCreateAimodel(doer *models.User, u *models.User, aimodel *models.AiModelManage) {
	...
	if err := models.NotifyWatchers(&models.Action{
		ActUserID: doer.ID,
		ActUser:   doer,
		OpType:    models.ActionCreateAimodel,
		AimodelID: aimodelId,
		Aimodel:   aimodel,
		IsPrivate: aimodel.IsPrivate,
		Content:   fmt.Sprintf("%s|%s", aimodel.ID, aimodel.DisplayName()),
	}); err != nil {
		log.Error("notify watchers(aimodel) '%d/%d': %v", doer.ID, aimodel.ID, err)
	}
}

func (*actionNotifier) NotifyDeleteAimodel(doer *models.User, u *models.User, aimodel *models.AiModelManage) {
	var aimodelId *string
	if aimodel.ID != "" {
		aimodelId = &aimodel.ID
	}
	if err := models.NotifyWatchers(&models.Action{
		ActUserID: doer.ID,
		ActUser:   doer,
		OpType:    models.ActionDeleteAimodel,
		AimodelID: aimodelId,
		Aimodel:   aimodel,
		IsPrivate: aimodel.IsPrivate,
		Content:   fmt.Sprintf("%s|%s", aimodel.ID, aimodel.DisplayName()),
...
func (t *actionNotifier) NotifyAimodelRecommend(optUser *models.User, aimodel *models.AiModelManage) {
	var aimodelId *string
	if aimodel.ID != "" {
		aimodelId = &aimodel.ID
	}
	log.Info("NotifyDeleteAimodel: aimodel.ID=%s, AimodelID=%v", aimodel.ID, aimodelId)
	act := &models.Action{
		OpType:    models.ActionAimodelRecommended,
		ActUserID: aimodel.OwnerID,
		AimodelID: aimodelId,
		IsPrivate: aimodel.IsPrivate,
		Content:   fmt.Sprintf("%s|%s", aimodel.ID, aimodel.DisplayName()),
	}
