...
`, map[string]string{
		...
		if ctx.User != nil {
			...
			ctx.Data["IsOperator"] = role.UserHasOper(ctx.User.ID, role.ROLE_OPER_KANBANAdmin)
			//ctx.User.IsOperator
			ctx.Data["IsNewAgree"] = models.GetNewAgreeByUID(ctx.User.ID)
			c.Data["SignedUserName"] = ctx.User.Name
			ctx.Data["IsMonitorAdmin"] = role.UserHasOper(ctx.User.ID, role.ROLE_OPER_MonitorAdmin)
		} else {
