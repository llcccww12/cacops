...
type Organization struct {
	...
	CanCreateOrgDataset bool
	CanCreateOrgAimodel bool
...
func HandleOrgAssignment(ctx *Context, args ...bool) {
	...
	if ctx.IsSigned && ctx.User.IsAdmin {
		...
		ctx.Org.CanCreateOrgDataset = true
		ctx.Org.CanCreateOrgAimodel = true
		...
		if ctx.Org.IsOwner {
			...
			ctx.Org.CanCreateOrgDataset, err = org.CanCreateOrgDataset(ctx.User.ID)
			if err != nil {
				ctx.ServerError("CanCreateOrgDataset", err)
				return
			}
			ctx.Org.CanCreateOrgAimodel, err = org.CanCreateOrgAimodel(ctx.User.ID)
			if err != nil {
				ctx.ServerError("CanCreateOrgAimodel", err)
				return
			}
		}
	} else {
	...
	ctx.Data["CanCreateOrgDataset"] = ctx.Org.CanCreateOrgDataset
	ctx.Data["CanCreateOrgAimodel"] = ctx.Org.CanCreateOrgAimodel
