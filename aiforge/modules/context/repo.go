...
func subjectAccessAssignment(ctx *Context, subjectAccessContext *models.SubjectAccessContext) {
	var err error
	if err = subjectAccessContext.GetOwner(); err != nil {
		ctx.ServerError("GetOwner", err)
		return
	}

	ctx.AccessContext.Permission, err = models.GetUserSubjectPermission(subjectAccessContext, ctx.User)
	if err != nil {
		ctx.ServerError("GetUserSubjectPermission", err)
		return
	}

	// Check access.
	if ctx.AccessContext.Permission.AccessMode == models.AccessModeNone {
		ctx.NotFound("no access right", nil)
		return
	}

	ctx.AccessContext.SubjectAccessContext = subjectAccessContext
}
...
func LowestLimiter() macaron.Handler {
	return limiterHandler(setting.RateLimitConfig.LowestRate, setting.RateLimitConfig.LowestBurst)
}
...
func AccessAssignment() macaron.Handler {
	return func(ctx *Context) {
		...
		if err != nil {
			ctx.ServerError("GetSubjectContext", err)
			return
		}
		...
		subjectAccessAssignment(ctx, subjectContext)
		if ctx.Written() {
			return
		}
		ctx.Next()
...
func RepoRefByType(refType RepoRefType) macaron.Handler {
	return func(ctx *Context) {
		...
		if ctx.Repo == nil || ctx.Repo.Repository == nil || ctx.Repo.Repository.IsEmpty {
			return
		}
		var (
			refName string
