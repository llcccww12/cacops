...
func NewStaticHandler(file string, dir string, opts *Options, ctx *macaron.Context) {
	opts.newstaticHandler(file, dir, ctx)
}

func (opts *Options) newstaticHandler(file string, dir string, ctx *macaron.Context) {
	// Defaults
	if len(opts.IndexFile) == 0 {
		opts.IndexFile = "index.html"
	}
	...
	opts.newhandle(file, ctx, opts)

}

func (opts *Options) newhandle(file string, ctx *macaron.Context, opt *Options) bool {
	...
	log.Println("avatar file2=" + file + "  opt.Prefix=" + opt.Prefix)
