...
// New creates a new GitLink provider and sets up important connection details.
// You should always call `gitlink.New` to get a new provider.  Never try to
// create one manually.
...
func NewCustomisedURL(clientKey, secret, callbackURL, authURL, tokenURL, profileURL string, scopes ...string) *Provider {
	p := &Provider{
		...
		providerName: setting.GitlinkConfig.Provider,
...
func (p *Provider) FetchUser(session goth.Session) (goth.User, error) {
	...
	log.Warn("gitlink:" + p.profileURL + "?access_token=" + url.QueryEscape(sess.AccessToken))
...
func userFromReader(r io.Reader, user *goth.User) error {
	...
	user.Provider = setting.GitlinkConfig.Provider
	user.Location = u.Phone
	return nil
}
