...
import (
	...
	"code.gitea.io/gitea/modules/auth/oauth2/providers/educoder"
	"code.gitea.io/gitea/modules/auth/oauth2/providers/gitlink"
	"code.gitea.io/gitea/modules/auth/oauth2/providers/osredm"
...
func createProvider(providerName, providerType, clientID, clientSecret, openIDConnectAutoDiscoveryURL string, customURLMapping *CustomURLMapping) (goth.Provider, error) {
	...
	case "educoder":
		authURL := setting.EduCoderConfig.AuthUrl
		tokenURL := setting.EduCoderConfig.TokenUrl
		profileURL := setting.EduCoderConfig.ProfileUrl
		provider = educoder.NewCustomisedURL(clientID, clientSecret, callbackURL, authURL, tokenURL, profileURL)
	case "gitlink":
		authURL := setting.GitlinkConfig.AuthUrl
		tokenURL := setting.GitlinkConfig.TokenUrl
		profileURL := setting.GitlinkConfig.ProfileUrl
		provider = gitlink.NewCustomisedURL(clientID, clientSecret, callbackURL, authURL, tokenURL, profileURL)
	case "osredm":
		authURL := setting.OsredmConfig.AuthUrl
		tokenURL := setting.OsredmConfig.TokenUrl
		profileURL := setting.OsredmConfig.ProfileUrl
		provider = osredm.NewCustomisedURL(clientID, clientSecret, callbackURL, authURL, tokenURL, profileURL)
	case "nextcloud":
