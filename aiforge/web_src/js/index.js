...
(function() {
  // Readme部分移动端左侧导航默认收缩
  const view_width = window.innerWidth || document.documentElement.clientWidth
  if(view_width < 767) {
    var toggle_container = document.getElementsByClassName('toggle-container');
    if (toggle_container.length > 0 && toggle_container[0].parentElement) {
      toggle_container[0].parentElement.classList.add('hidden')
      toggle_container[0].querySelector('.icon').classList.replace('ri-arrow-drop-left-line','ri-arrow-drop-right-line')
    }
  }
...
function initBranchSelector() {
  ...
  $selectBranch.find(".reference.column").on("click", function () {
    ...
    $(this).find(".text.black").removeClass("black");
    return false;
  });
}
        ...
        retrieveImageFromClipboardAsBlob(event, (img) => {
          ...
          uploadFile(img, (res) => {
            ...
            replaceAndKeepCursor(
              field,
              `![${name}]()`,
              `![${name}](${AppSubUrl}/attachments/${data.uuid})`
            );
            const input = $(
      ...
      switch (input_id) {
        case "#milestone_id":
          $list
            .find(".selected")
            .html(
              `<a class="item" href=${$(this).data("href")}>${htmlEncode(
                $(this).text()
              )}</a>`
            );
          break;
        case "#assignee_id":
          $list
            .find(".selected")
            .html(
              `<a class="item" href=${$(this).data("href")}>` +
                `<img class="ui avatar image" src=${$(this).data(
                  "avatar"
  ...
  if ($(".organization.settings.options").length > 0) {
    $("#org_name").on("keyup", function () {
      ...
      if (
        ...
        $(this).data("org-name").toString().toLowerCase()
      ) {
        $prompt.show();
      } else {
        $prompt.hide();
...
function initUserSettings() {
  ...
  if ($(".user.settings.profile").length > 0) {
    $("#username").on("keyup", function () {
      ...
      if (
        $(this).val().toString().toLowerCase() !==
        $(this).data("name").toString().toLowerCase()
      ) {
        $prompt.show();
      } else {
        $prompt.hide();
...
function initGithook() {
  ...
  CodeMirror.autoLoadMode(
    CodeMirror.fromTextArea($("#content-githook")[0], {
      lineNumbers: true,
      mode: "shell",
    }),
    "shell"
...
function searchDataset() {
  const $searchRepoBox = $("#search-dataset-box");
  const orgname = $searchRepoBox.data('name');
  const type = $searchRepoBox.data('type');
  $searchRepoBox.search({
    minCharacters: 2,
    apiSettings: {
      url: `${AppSubUrl}/api/v1/orgs/${orgname}/${type}/list?_csrf=${csrf}&q={query}`,
      onResponse(response) {
        const items = [];
        let res = type === 'aimodel' ? response.data.aimodels : response.data.datasets;
        $.each(res, (_i, item) => {
          items.push({
            title: item.alias,
            id: item.id,
            description: `${orgname}/${item.name}`,
          });
        });
    ...
    onSelect: function(result, response) {
      setSelectedDataset(result.title, result.id);
      return true;
    }
  });
  function setSelectedDataset(name, id) {
    $('#dataset-name-display').val(name);
    $('#dataset-id-value').val(id);
    
    // 添加选中状态标记
    $('#dataset-name-display').data('valid-selection', true);
  }
  // 监听输入变化 - 当用户修改输入时清除ID
  $('#dataset-name-display').on('input', function() {
    const $this = $(this);
    
    // 只有当用户实际修改内容时才清除
    if ($this.val() !== $this.data('last-value')) {
      // 清除隐藏的ID值
      $('#dataset-id-value').val('');
      // 标记为无效选择
      $this.removeData('valid-selection');
    }
  ...
  // 初始化状态
  $('#dataset-name-display').data('last-value', '');
  $('#dataset-id-value').val('');
}
  ...
  Vue.component("repo-search", {
    ...
    methods: {
      ...
      updateHistory() {
        ...
        // window.history.replaceState({}, "", params.toString() ? `?${params.toString()}` : "dashboard");
      },
...
function initVueDataset() {
  ...
  const upgradeTips = $('#dataset-base').data('upgrade-tip')
  const notExistTips = $('#dataset-base').data('not-exist')
  const items = [];
  const zipStatus = [];
  $("#dataset-range-value")
    .find(".item")
  ...
  new Vue({
    ...
    methods: {
      ...
      downloadSingleData(url,size,s3url,dataset_name,max_model_size,format_size) {
        if (size > max_model_size) {
          // let sdkUrl = `/api/v1${repolink}/sdk/generate_dataset_download_code?dataset_file_name=${dataset_name}`
          // this.$axios.get(sdkUrl).then((res) => {
          //   res = res.data
          //   if (res.code == 0) {
          //     let contentStr = i18n['data_sdk_tips1'].format(format_size)
          //     contentStr += `\n\`\`\`python\n${res.data.code}\`\`\`\n\n${i18n['modelObj']['model_sdk_tips2']}\n\`\`\`bash\n${res.data.cli}\n\`\`\`\n\n`;
          //     contentStr += i18n['data_sdk_tips3']
          //     this.getMarkdownHtml(contentStr)
          //   } else {
      ...
      getMarkdownHtml(context) {
        const params = { mode: 'gfm', text: context, __csrf: csrf }
        this.$axios.post('/api/v1/markdown', params).then((res) => {
          const html = res.data;
          this.contentSDK = this.hljsAndInsertCopyButton(html);
          this.$nextTick(() => {
      ...
      hljsAndInsertCopyButton(htmlStr) {
        ...
        for (let i = 0, iLen = codeBlocks.length; i < iLen; i++) {
          ...
          copyBtn.innerHTML = `<span style="color: #0366d6;cursor: pointer;" class="ui poping inline up clipboard" id="clipboard-${this.sparkMD5Hash(txt)}"
              data-position="top center" data-variation="inverted tiny" data-success="${i18n['cloudeBrainMirror']['copy_succeeded']}"
              data-content="${i18n['cloudeBrainMirror']['copy']}" data-original="${i18n['cloudeBrainMirror']['copy']}" 
      ...
      setUpdate(uuid) {
        this.loading = true
        this.$axios.post(`/api/v1/dataset/historic/one?uuid=${uuid}&_csrf=${csrf}`).then((res) => {
          console.log(res);
          res = res.data
          if (res.code == 0) {
            location.reload();
            this.$message.success(upgradeTips)
          }
          // $('#'+uuid).hide()
          this.loading = false
        })
      ...
      goToNewVesion(ownerName, name) {
        if (ownerName && name) {
          location.href = `/datasets/detail/${ownerName}/${name}`
        } else {
          this.$message.error(notExistTips)
        }
          ...
          const items = vm.items.filter((item) => {
            return (
              ((vm.mode === "branches" && item.branch) ||
                (vm.mode === "tags" && item.tag)) && (!vm.searchTerm ||
...
function initNavbarContentToggle() {
  const content = $("#navbar-expand-content");
  ...
  $('.navbar-item-toggle-content').hide();
  $('.navbar-item-toggle').on('click', function(evt) {
    const self = $(this);
    if (evt.target.nodeName == 'A') return;
    const content = self.next('.navbar-item-toggle-content');
    const triangle = self.find('.triangle');
        ...
        $.each(response, (_i, issue) => {
          ...
          filteredResponse.results.push({
            name: `#${issue.number} ${htmlEncode(
              issue.title
            )}<div class="text small dont-break-out">${htmlEncode(
