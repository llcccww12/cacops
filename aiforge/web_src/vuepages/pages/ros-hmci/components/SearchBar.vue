    ...
    <div class="_repo_search">
      ...
      <div class="_repo_search_label_c">
        <div class="_repo_search_label">
          <a v-if="type == 'square'" :href="`/explore/repos?q=${searchInputValue.trim()}&topic=${item.v
            }&sort=${sort}`" :style="{ backgroundColor: topicColors[index % topicColors.length] }"
            v-for="(item, index) in topics" :key="index">{{ item.v }}</a>
          <a v-if="type == 'search'" href="javascript:;" @click="changeTopic({ k: '', v: '' })" style="font-weight: bold"
            :style="{
              backgroundColor: selectTopic == '' ? selectedColor : defaultColor,
              color: selectTopic == '' ? 'white' : '#40485b',
            }">{{ $t("repos.allFields") }}</a>
          <a v-if="type == 'search'" href="javascript:;" @click="changeTopic(item)" :style="{
            backgroundColor:
              selectTopic.toLocaleLowerCase() == item.k ? selectedColor : defaultColor,
            color: selectTopic.toLocaleLowerCase() == item.k ? 'white' : '#40485b',
          }" v-for="(item, index) in topics" :key="index">{{ item.v }}</a>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      if (this.type == "square") {
        window.location.href = `/explore/repos?q=${this.searchInputValue}&sort=${this.sort}&topic=ros-hmci-app`;
      } else {
        this.$emit("change", {
  ...
  mounted() {
    if (this.static) {
      ...
      getPromoteData("/repos/recommend_topics")
        .then((res) => {
          const data = res.data;
