      ...
      <template slot="right">
        <el-button class="op-btn" type="primary" @click="goCreate">
          <div class="btn-content">
            ...
            <span>{{$t('modelSquare.newSftFinetune')}}</span>
          </div>
        </el-button>
      </template>
    </Header>
        ...
        <div class="body">
          <div class="table-container">
            <el-table :data="tableData" style="min-width:100%" height="100%" v-loading="loading" stripe>
              ...
              <el-table-column :label="$t('modelManage.modelName')" align="left" header-align="left" min-width="250">
                <template slot-scope="scope">
                  <span>{{ scope.row.task.app_name }}</span>
                </template>
              </el-table-column>
              <el-table-column prop="Status" :label="$t('status')" align="left" header-align="center" min-width="142">
                <template slot-scope="scope">
              ...
              <el-table-column prop="remark" :label="$t('operation')" align="left" min-width="200" header-align="center"
                fixed="right">
                <template slot-scope="scope">
                  <div class="op-wrap">
                    <a href="javascript:;" v-if="scope.row.can_fintune_experience"
                      @click="opDeploy(scope.row)">{{ $t('cloudbrainObj.deploymentExperience') }}</a>
...
import { getMyAiTasks, stopAiTask, deleteAiTask } from '~/apis/modules/cloudbrain';
...
export default {
  ...
  methods: {
    ...
    goDetail(row) {     
      this.$router.push(this.$route.path + `/detail/${row.task.id}`);
    },
    opDeploy(row){
      const item = row.task
