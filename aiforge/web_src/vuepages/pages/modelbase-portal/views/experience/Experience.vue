      ...
      <template slot="right">
        <el-button class="op-btn" type="primary" @click="goCreate">
          <div class="btn-content">
            ...
            <span>{{ $t('modelSquare.createChatBtns1') }}</span>
          </div>
        </el-button>
      </template>
    </Header>
        ...
        <div class="body">
          <div class="table-container">
            <el-table :data="tableData" style="min-width:100%" height="100%" v-loading="loading" stripe>
              ...
              <el-table-column prop="remark" :label="$t('operation')" align="left" min-width="250" header-align="center"
                fixed="right">
                <template slot-scope="scope">
                  <div class="op-wrap">
                    <a href="javascript:;"
                      :class="scope.row.can_experience && scope.row.task.canDebug ? '' : 'disabled'"
                      @click="opOnline(scope.row)">{{ $t('modelManage.onlineInference') }}</a>
...
export default {
  ...
  methods: {
    ...
    goDetail(row) {
      this.$router.push(`/experience/detail/${row.task.id}`);
    },
    opStop(row) {
