      ...
      <div class="main-content">
        ...
        <div v-if="!alreadyMsgBoxShow" class="create-tips">
          ...
          <span style="margin-left:4px;">{{ $t('cloudbrainObj.runningLimit', {
            count: limitCount, taskType:
              $t('modelManage.onlineInferenceTask')
          }) }}</span>
        </div>
        ...
        <div class="area" v-loading="globalLoading">
          <div class="area-content">
            ...
            <TaskName ref="taskNameRef" v-model="state.taskName" autofocus :required="true" :userName="loginName">
            </TaskName>
...
export default {
  ...
  components: { Header, LoadingMask, FormTopV2, SpecSelect, TaskName, AcknowledgementsTips, ForbidPenetrationTipCheck, TaskLimitDialog },
  computed: {
    limitDialogTitle() {
      return this.$t('cloudbrainObj.runningLimit', { count: this.limitCount, taskType: this.$t('modelManage.onlineInferenceTask') });
    },
    limitDialogDesc() {
  ...
  methods: {
    ...
    // TaskLimitDialog 事件处理
    onLimitStopSuccess() {
      this.prepare();
    },
    onLimitDeleteSuccess() {
