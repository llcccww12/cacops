      ...
      <template slot="right">
        <el-button class="op-btn" type="primary" @click="goCreate">
          <div class="btn-content">
            ...
            <span>{{ $t('modelSquare.createEvalBtns1') }}</span>
          </div>
        </el-button>
      </template>
    </Header>
    <div class="tips" v-html="$t('modelFinetune.maxEvalTaskTips')"></div>
    <div class="main-body">
      <div class="content">
        <div class="body">
          <div class="table-container">
            <el-table :data="tableData" style="min-width:100%" height="100%" v-loading="loading" stripe>
              ...
              <el-table-column :label="$t('modelSquare.evaluationModel')" align="left" header-align="left" min-width="250">
                <template slot-scope="scope">
                  <span>{{ scope.row.task.app_name }}</span>
                </template>
              </el-table-column>
              <el-table-column prop="Status" :label="$t('status')" align="left" header-align="center" min-width="150">
                <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    goDetail(row) {
      this.$router.push(`/eval/evaluate/detail/${row.task.id}`);
    },
    opStop(row) {
