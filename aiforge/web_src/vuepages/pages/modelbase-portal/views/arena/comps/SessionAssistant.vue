<template>
  <div>
        ...
        <div class="chat-output-wrapper">
          <div class="chat-container">
            <div style="overflow: hidden;background-color: transparent;height: auto;">
              <div class="loading" v-if="useLoading && !htmlContent && data.loading"><i class="el-icon-loading"></i>
              </div>
    ...
    <!-- <div v-if="!!htmlContent && data.loading" class="chat-c2net-stop" @click="$emit('stopGeneration')"><el-button
        style="font-size: 12px;" type="text">{{ $t('modelSquare.stopChat') }}</el-button></div> -->
...
import createRenderer from '~/pages/model/llms/componenes/render.js'
...
.chat-main-content {
  ...
  .chat-output-wrapper {
    ...
    background: rgba(75, 148, 205, 0.1);
