...
<div style="margin: 0 20px">
  <div class="title"><span>{{$t('userRole.UserPermissionConfig')}}</span></div>
  <div style="margin-top: 15px;">
    <el-input :placeholder="$t('userRole.pleaseEnterContent')" size="medium" v-model="searchValue" class="input-with-select" @keydown.enter.stop.native.prevent="searchUser">
      <el-select v-model="select" slot="prepend">
        <el-option :label="$t('userRole.userName')" value="1"></el-option>
        <el-option :label="$t('userRole.userId')" value="2"></el-option>
      </el-select>
  ...
  <div class="table-container">
    <div style="width: 100%">
      <el-table border :data="filtedData" v-tableSticky style="width: 100%" v-loading="loading" stripe>
        ...
        <el-table-column prop="Name" fixed :label="$t('userRole.userName')" width="200">
        ...
        <el-table-column prop="CreateTime" :label="$t('modelManage.createTime')" align="center" width="160" />
        <el-table-column prop="UpdatedTime" :label="$t('modelManage.updateTime')" align="center" width="160" />
...
export default {
  data() {
    return {
      ...
      sortSelectList:[
        ...
        {k:'created_unix asc', v:this.$t('userRole.permissionCreatedUnixAsc')},
        {k:'created_unix desc', v:this.$t('userRole.permissionCreatedUnixDesc')},
      ],
      sortSelect:'',
      tableData:[],
      loading:false,
  ...
  methods: {
    ...
    openEdit(item){
      window.location.href =`/admin/access/${item.ID}?name=${item.Name}&opRole=${item.operRolesIds}&reRole=${item.resourceRolesIds}&stRole=${item.storageRolesIds}`
    },
    initRoleList(){
      listAiforgeRole({}).then(res=>{
        const data = res.data
        data.forEach((item)=>{
          if(item.Type===0){
    ...
    initRightUserList(){
      this.loading = true
      listRightUser(this.params).then((res)=>{
        if (!res?.data) {
