<template>
  <div class="list-container">
    <div class="list-head">
      ...
      <div class="title-r">
        <div class="search-c">
          <el-input v-model="q" :placeholder="$t('org.searchModels')" @keyup.native.enter="search">
    ...
    <div class="list-item-container" v-loading="loading">
      <div class="item-container" v-for="(item, index) in list" :key="item.ID">
        <DatasetItem :data="item" :canChangeFav="true" type="aimodel" @changeFav="onChangeFav"></DatasetItem>
      </div>
...
import { getOrgModelList, getOrgLabel } from '~/apis/modules/organization';
...
export default {
  ...
  methods: {
    ...
    getListData() {
      ...
      getOrgModelList(requestParams).then(res => {
        res = res.data;
        this.loading = false;
        this.total = res.data.total || 0;
        this.list = (res.data.aimodels || []).map(item => {
          return {
            ...item,
    ...
    getLabelList(){
      getOrgLabel({ orgName: this.pageData.Org.Name}, 'aimodel').then((res)=>{
        res = res.data
        if(res.code === 0){
          console.log(res.data)
          const orgTopics = res.data;
          const engine = (orgTopics.Engine || []).map(item => {
            return {
              k: [`engine|${item}`],
              v: getListValueWithKey(MODEL_ENGINES, item),
            }
          })
          const label = (orgTopics.Label || []).filter(item => item.trim() != '').map(item => {
            return {
              k: [`label|${item}`],
              v: item,
            }
