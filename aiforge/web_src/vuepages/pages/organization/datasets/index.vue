  ...
  <div class="content">
    <div class="content-r">
      <div class="filter-c-m">
        <span class="title">{{ $t('org.orgDataset')}}</span>
        <div class="right-m">
          ...
          <div v-else  class="ui small icon input" style="height: 32px;margin-left: 6px;">
            <input type="text" :placeholder="$t('org.searchDatasets')" v-model="q" @keyup.enter="search">
            <i class="search icon" style="cursor: pointer;pointer-events: auto;" @click="search"></i>
          </div> 
        </div>
      </div>
      <div class="workspace-t">
        <span>{{ $t('org.orgDataset')}}</span>
      </div>
      <div class="filter-c">
        <div class="tab-c">
        ...
        <div class="right">
          ...
          <div class="btn-add" v-if="canCreate" @click="createDataset"> 
            ...
            <span style="margin-left:6px;">{{ $t('datasetObj.create_new_dataset')  }}</span>
          </div>
        </div>
      </div>
      <div class="list-item-container" v-loading="loading">
        <div class="item-container" v-for="(item, index) in list" :key="item.id">
          <DatasetItem :data="item" :canChangeFav="true" :operaFlag="true" @deleteEvent="deleteEvent"></DatasetItem>
        </div>
...
import { getOrgDatasetList } from "~/apis/modules/organization";
...
export default {
  ...
  methods: {
    ...
    createDataset(){
      location.href = `/guide/create_dataset?org=${this.uid}`
    },
    search(){
      this.iPage = 1;
      this.getCardList()
    ...
    async getCardList() {
      try {
        ...
        const response = await getOrgDatasetList(requestParams)
        this.loading = false;
