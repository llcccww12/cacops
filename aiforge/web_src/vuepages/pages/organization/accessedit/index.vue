...
<div style="margin: 0 20px;height: 100%;" v-loading="loading">
  <div class="page-title"><span>{{$t('userRole.editOrgPermissions')}}：{{name}}</span></div>
  ...
  <div style="display:flex">
    ...
    <div class="btn-wrap" @click="cancel" style="margin-left:1rem;background:#c2c7cc;"><span style="color: #020004;">{{$t('cancel')}}</span></div>
  </div>
  
  <!-- <div class="resource-title">
    <span>{{name}} {{$t('userRole.resourcePermissions')}}：</span>
  </div>
  <resourceTable v-if="Object.keys(showSpecTableData).length" ref="resourceTable" :showSpecTableData="showSpecTableData" :specificationList="specificationList"></resourceTable>
...
import opRoleDialog from '~/pages/user/role/opRoleDialog.vue';
import resourceRoleDialog from '~/pages/user/role/resourceRoleDialog.vue';
import storageRoleDialog from '~/pages/user/role/storageRoleDialog.vue';
...
import { listAiforgeOrgRole, setAiforgeRoleToOrgUser } from '~/apis/modules/organization';
...
export default {
  ...
  methods: {
    ...
    initRoleList(){
      return listAiforgeOrgRole({}).then((res)=>{
        const data = res.data
    ...
    updateRole(){
      ...
      setAiforgeRoleToOrgUser(data).then((res)=>{
        if(res.data.code==='0'){
