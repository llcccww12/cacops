      ...
      <div class="filter-panel">
        ...
        <div class="filter-section">
          ...
          <div class="filter-buttons">
            ...
            <div class="search-button btn" @click="searchFilters">{{$t('repos.search')}}</div>
          </div>
        </div>
...
import { getModelSqaureFilters, getModelList } from '~/apis/modules/modelsquare';
import { delDataset, getPromoteDataset } from "~/apis/modules/dataset";
...
export default {
  data() {
    return {
      ...
      tabList1: [
        { key: '/public?recommend=all&aimodel_type=', label: `⭕ ${this.$t('modelObj.model_public')}`, favFlag: true },
        { key: '/public?recommend=only&aimodel_type=', label: `🏆 ${this.$t('modelObj.model_recommend')}`, favFlag: true },
        { key: '/public?recommend=all&aimodel_type=2', label: `✈️ ${this.$t('modelObj.model_migrate')}`, favFlag: true, btnUrl: '/repo/model_migrate', btnText: this.$t('modelObj.migrate_external_model') },
        { key: '/modelscope', label: '🌐 魔塔社区', favFlag: false, modelscope: true },
      ],
      tabList2: [
        { key: '/owned?aimodel_type=', label: this.$t('modelObj.model_my'), favFlag: false, operaFlag: true },
        { key: '/collaborated?aimodel_type=', label: this.$t('modelObj.model_collaborate'), favFlag: false },
        { key: '/collected?aimodel_type=', label: this.$t('modelObj.model_collected'), favFlag: true, reloadFlag: true },
        { key: '/involved?aimodel_type=2', label: this.$t('modelObj.model_my_migrate'), favFlag: false, btnUrl: '/repo/model_migrate', btnText: this.$t('modelObj.migrate_external_model')},
      ],
      ...
      name: '',
      delLoading: false,
      codeUsePromotePath: `home_v2/model_square${lang == 'zh-CN' ? '' : '_en'}.json`,
    };
  },
    ...
    async getCardList() {
        try {
            ...
            if (this.params.url === '/modelscope') {
              const response = await fetch(`/api/v1/modelscope/models?page=${this.params.page || 1}&page_size=${this.params.page_size || 30}&q=${encodeURIComponent(this.params.q || '')}&order_by=${encodeURIComponent(this.params.order_by || '')}`)
              const payload = await response.json()
              this.loading = false
              if (payload.code === 0) {
                const res = payload.data || {}
                this.list = (res.models || []).map(item => ({
                  ...item,
                ...
                this.$message.error(payload.msg || '加载魔塔社区模型失败')
              }
              return
            }
            // 处理 URL 中的重复参数
            ...
            const response = await getModelList(requestParams)
            this.loading = false;
    ...
    async getDatasetTags() {
        try {
          ...
          if (response.data.code === 0){
            ...
            const {frame = {}, label = {}} = res.tagInformation || {}
            ...
            const frameTag = createTagArray(frame);
            const labelTag = createTagArray(label);
            this.filtersConfig = [
              {
                  type: 'engine',
