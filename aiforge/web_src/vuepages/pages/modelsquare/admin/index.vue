  ...
  <div>
    ...
    <div>
        ...
        <div style="display: flex;justify-content: space-between;align-items: center;margin:15px 0">
          <el-select style="width: 10%" size="medium" v-model="visibility" @change="selectVisibilityChange">
    ...
    <div class="ui attached table segment">
        <el-table :data="list" style="width: 100%;font-size: 14px;" stripe v-loading="loading">
            <el-table-column  prop="name" :label="$t('repos.model')" width="500">
                <template slot-scope="scope">
                    <div style="display: flex;align-items: center;">
                        <a style="margin-right:8px" class="nowrap" :href="`/models/detail/${scope.row.owner_name}/${scope.row.name}`" target="_blank">{{scope.row.alias}}</a>
  ...
  methods: {
    ...
    async getDatasetList() {
      try {
            ...
            const response = await getAdminDatasets(this.params,'aimodel')
            this.loading = false;
            
            if(response.data.code === 0){
              const res = response.data.data
              console.log("res.aimodels",res.aimodels)
    ...
    async recommendData(item){
        ...
        try { 
            ...
            const response = item.recommend ? await delRecommendDatasets(params,'aimodel') : await putRecommendDatasets(params,'aimodel')
