...
func (repo *Repository) GetAITemplateEditURL(branchNames ...string) string {
	var branchName = repo.DefaultBranch
	if len(branchNames) > 0 {
		branchName = branchNames[0]
	}
	return repo.Link() + "/_edit/" + util.PathEscapeSegments(branchName) + "/" + setting.AI_TASK_TEMPLATE_FILE_NAME
}

func (repo *Repository) GetAITemplateNewURL(branchNames ...string) string {
	var branchName = repo.DefaultBranch
	if len(branchNames) > 0 {
		branchName = branchNames[0]
	}
	return repo.Link() + "/_new/" + util.PathEscapeSegments(branchName)
}
...
func (repo *Repository) updateSize(e Engine) error {
	...
	archiveSize, _ := util.GetDirectorySize(path.Join(repo.RepoPath(), "archives"))
	repo.Size = size - archiveSize
	...
	if models != nil {
		for _, model := range models {
			...
			if model.ModelType == 2 {
				err := DeleteHfFilesByModelId(model.ID)
				if err != nil {
...
func getDatasetCount(e Engine, u *User) (int64, error) {
	return e.Count(&DatasetRegistry{OwnerID: u.ID})
}

func getAimodelCount(e Engine, u *User) (int64, error) {
	return e.Count(&AiModelManage{OwnerID: u.ID})
}
