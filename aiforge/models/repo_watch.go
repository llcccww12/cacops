...
func notifyWatchers(e Engine, actions ...*Action) error {
	...
	for _, act := range actions {
		if act.RepoID == 0 && act.DatasetID != nil && !act.IsCloudbrainAction() {
			continue
		}
		if act.RepoID == 0 && act.AimodelID != nil && !act.IsCloudbrainAction() {
			continue
		}
...
func notifyCloudbrainsWatchers(e Engine, actions ...*Action) error {

	var err error

	for _, act := range actions {
		if !act.IsCloudbrainAction() {
			continue
		}
...
func NotifyWatchers(actions ...*Action) error {
	...
	if err := notifyWatchers(x, actions...); err != nil {
		errs = append(errs, fmt.Sprintf("notifyWatchers: %w", err))
	}
	if err := notifyDatasetWatchers(x, actions...); err != nil {
		errs = append(errs, fmt.Sprintf("notifyDatasetWatchers: %w", err))
	}
	if err := notifyAimodelWatchers(x, actions...); err != nil {
		errs = append(errs, fmt.Sprintf("notifyAimodelWatchers: %w", err))
	}
	producer(actions...)
	if len(errs) > 0 {
