...
func (p *Permission) IsReader() bool {
...
func GetUserSubjectPermission(ctx *SubjectAccessContext, user *User) (Permission, error) {
	return getUserSubjectPermission(x, ctx, user)
}
...
func getUserSubjectPermission(e Engine, ctx *SubjectAccessContext, user *User) (perm Permission, err error) {
	if log.IsTrace() {
		defer func() {
			if user == nil {
				log.Trace("Permission Loaded for anonymous user in %-v:\nPermissions: %-+v",
	...
	var isCollaborator bool
	if user != nil {
		isCollaborator, err = ctx.isCollaborator(e, user.ID)
		if err != nil {
			return perm, err
		}
	}
	...
	if ctx.Owner.IsOrganization() && !HasOrgVisible(ctx.Owner, user) && !isCollaborator {
		perm.AccessMode = AccessModeNone
		return
	}
	...
	if user.IsAdmin || user.ID == ctx.OwnerID {
		perm.AccessMode = AccessModeOwner
		return
	}

	// plain user
	perm.AccessMode, err = subjectAccessLevel(e, user, ctx)
	if err != nil {
		return
	}
	...
	teams, err := getUserSubjectTeams(e, ctx.OwnerID, user.ID, ctx.SubjectID, ctx.SubjectType)
	if err != nil {
		return
	}
...
func hasSubjectAccess(e Engine, userID int64, ctx *SubjectAccessContext) (bool, error) {
	var user *User
	var err error
	if userID > 0 {
		user, err = getUserByID(e, userID)
	...
	perm, err := getUserSubjectPermission(e, ctx, user)
	if err != nil {
		return false, err
	}
	return perm.HasAccess(), nil
