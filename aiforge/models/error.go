...
type ErrUserOwnDatasets struct {
	UID int64
}

// IsErrUserOwnRepos checks if an error is a ErrUserOwnRepos.
func IsErrUserOwnDatasets(err error) bool {
	_, ok := err.(ErrUserOwnDatasets)
	return ok
}

func (err ErrUserOwnDatasets) Error() string {
	return fmt.Sprintf("user still has ownership of datasets [uid: %d]", err.UID)
}

type ErrUserOwnAimodels struct {
	UID int64
}

// IsErrUserOwnRepos checks if an error is a ErrUserOwnRepos.
func IsErrUserOwnAimodels(err error) bool {
	_, ok := err.(ErrUserOwnAimodels)
	return ok
}

func (err ErrUserOwnAimodels) Error() string {
	return fmt.Sprintf("user still has ownership of aimodels [uid: %d]", err.UID)
}

// ErrUserOwnRepos represents a "UserOwnRepos" kind of error.
type ErrUserHasCloudbrainTask struct {
	TaskName string
	RepoName string
}

// IsErrUserOwnRepos checks if an error is a ErrUserOwnRepos.
func IsErrUserHasCloudbrainTask(err error) bool {
	_, ok := err.(ErrUserHasCloudbrainTask)
	return ok
}

func (err ErrUserHasCloudbrainTask) Error() string {
	return fmt.Sprintf("user still has not stopped  [taskName: %s,repoName:%s]", err.TaskName, err.RepoName)
}
...
type ErrModelPublished struct {
}

func IsErrErrModelPublished(err error) bool {
	_, ok := err.(ErrModelPublished)
	return ok
}

func (err ErrModelPublished) Error() string {
	return fmt.Sprintf("Model has been published")
}
...
type ErrCannotStopSavingImageJob struct {
}

func IsErrCannotStopSavingImageJob(err error) bool {
	_, ok := err.(ErrCannotStopSavingImageJob)
	return ok
}
