...
type User struct {
	...
	RightUnix     timeutil.TimeStamp `xorm:"INDEX NOT NULL DEFAULT 0"`
	...
	NumAimodelStars int `xorm:"NOT NULL DEFAULT 0"`
	...
	DatasetAdminChangeTeamAccess bool                `xorm:"NOT NULL DEFAULT false"`
	AimodelAdminChangeTeamAccess bool                `xorm:"NOT NULL DEFAULT false"`
	...
	OperRole     []*AiforgeRole `xorm:"-"`
	ResourceRole []*AiforgeRole `xorm:"-"`
	StorageRole  []*AiforgeRole `xorm:"-"`
...
func (u *User) CanDeleteDatasetRegistry(dataset DatasetRegistry) bool {
	acessCtx := dataset.ConvertSubjectAccessContext()
	if u.IsAdmin || u.ID == acessCtx.OwnerID {
		return true
	}
	...
	if acessCtx.Owner.IsOrganization() {
		isOwner, err := acessCtx.Owner.IsOwnedBy(u.ID)
		if err != nil {
			return false
		} else if isOwner {
			return true
...
func (u *User) CanDeleteAiModelManage(aimodel AiModelManage) bool {
	acessCtx := aimodel.ConvertSubjectAccessContext()
	if u.IsAdmin || u.ID == acessCtx.OwnerID {
		return true
	}
...
func (u *User) UpdateRandomAvatarUseDiffustionModelByUser(imgBase64 string) error {
	img, data, err := avatar.Base64ToImage(imgBase64)
	if err != nil {
		return err
	}
...
func (u *User) UpdateRandomAvatarUseDiffustionModel() error {
	if len(setting.AIAvartarPromoteConfig.AnimalPrompt) == 0 {
		setting.GetDiffusionAvatarConfig()
	}
	img, data, err := avatar.RandomDiffusionImageSize(setting.AIAVartarSize, setting.AIAvartarPromoteConfig.Prompt[rand.Intn(len(setting.AIAvartarPromoteConfig.Prompt))])
	if err != nil {
...
func (u *User) GenerateRandomAvatarUseDiffustionModel() error {
	if len(setting.AIAvartarPromoteConfig.AnimalPrompt) == 0 {
		setting.GetDiffusionAvatarConfig()
	}
	img, _, err := avatar.RandomDiffusionImageSize(setting.AIAVartarSize, setting.AIAvartarPromoteConfig.Prompt[rand.Intn(len(setting.AIAvartarPromoteConfig.Prompt))])
	if err != nil {
		return fmt.Errorf("RandomImage: %v", err)
	}
	...
	if _, err := x.ID(u.ID).Cols("avatar").Update(u); err != nil {
		return err
	}
	if err = png.Encode(fw, img); err != nil {
		return fmt.Errorf("Encode: %v", err)
	...
	log.Info("New diffusion random avatar created: %d", u.ID)
	return nil
}
...
func (u *User) SizedRelAvatarLink(size int) string {
	return strings.TrimRight(setting.AppSubURL, "/") + "/user/avatar/" + u.Name + "/" + strconv.Itoa(size) + "?" + u.Avatar
}
...
func (u *User) UploadAvatar(data []byte) error {
	...
	customAvatarPath := u.CustomAvatarPath()
	log.Info("customAvatarPath=" + customAvatarPath)
	fw, err := os.Create(u.CustomAvatarPath())
	if err != nil {
		return fmt.Errorf("Create: %v", err)
	}
...
func deleteUser(e *xorm.Session, u *User) error {
	...
	count, err = getDatasetCount(e, u)
	if err != nil {
		return fmt.Errorf("getDatasetCount: %v", err)
	} else if count > 0 {
		return ErrUserOwnDatasets{UID: u.ID}
	}
	// Check ownership of aimodel.
	count, err = getAimodelCount(e, u)
	if err != nil {
		return fmt.Errorf("getAimodelCount: %v", err)
	} else if count > 0 {
		return ErrUserOwnAimodels{UID: u.ID}
	}
	...
	cloudbrains, err := GetCloudbrainRunByUserId(u.ID)
	if err == nil {
		if len(cloudbrains) > 0 {
			var reponame string
			repo, err1 := GetRepositoryByID(cloudbrains[0].RepoID)
			if err1 == nil {
				reponame = repo.FullDisplayName()
			}
			return ErrUserHasCloudbrainTask{TaskName: cloudbrains[0].DisplayJobName, RepoName: reponame}
		}
	}
	...
	collectedDatasetIDs := make([]string, 0, 10)
	if err = e.Table("dataset_collection").Cols("dataset_id").
		Where("user_id = ?", u.ID).Find(&collectedDatasetIDs); err != nil {
		return fmt.Errorf("get all dataset collection: %v", err)
	} else if _, err = e.Decr("num_collections").In("id", collectedDatasetIDs).NoAutoTime().Update(new(DatasetRegistry)); err != nil {
		return fmt.Errorf("decrease dataset num_collections: %v", err)
	}
	...
	if err = deleteBeans(e,
		...
		&SubjectCollaboration{UserID: u.ID},
		&SubjectAccess{UserID: u.ID},
		&DatasetCollection{UserID: u.ID},
	); err != nil {
...
func DeleteInactiveUsers(ctx context.Context, olderThan time.Duration) (err error) {
	...
	for _, u := range users {
		...
		if err = DeleteUser(u); err != nil {
			...
			if IsErrUserOwnRepos(err) || IsErrUserHasOrgs(err) || IsErrUserOwnDatasets(err) || IsErrUserOwnAimodels(err) {
				continue
			}
			return err
		}
...
func QueryUserName(intSlice []int64) map[int64]*User {
	...
	userLists, err := GetUsersByIDs(uniqueElements)
	if err == nil {
...
func GetWatchedRepos(userID int64, private bool, listOptions ListOptions, keyword string) ([]*Repository, error) {
	...
	if keyword != "" {
		sess = sess.And(builder.Like{"`repository`.lower_alias", "%" + strings.ToLower(keyword) + "%"})
	}
...
// GetWatchedReposCount returns the number of repos watched by a particular user,
// with the same filter conditions as GetWatchedRepos
func GetWatchedReposCount(userID int64, private bool, keyword string) (int64, error) {
	sess := x.Where("watch.user_id=?", userID).
		And("`watch`.mode<>?", RepoWatchModeDont).
		Join("LEFT", "watch", "`repository`.id=`watch`.repo_id")
	if !private {
	...
	return sess.Count(new(Repository))
}
...
type SignUpWechatCache struct {
	WechatOpenId string
}

type LoginWechatCache struct {
	UserId int64
}
