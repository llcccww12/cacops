...
func (org *User) CanCreateOrgDataset(uid int64) (bool, error) {
	return CanCreateOrgDataset(org.ID, uid)
}

func (org *User) CanCreateOrgAimodel(uid int64) (bool, error) {
	return CanCreateOrgAimodel(org.ID, uid)
}
...
func CreateOrganization(org, owner *User) (err error) {
	...
	t := &Team{
		...
		DatasetAuthorize:        AccessModeOwner,
		AimodelAuthorize:        AccessModeOwner,
		NumMembers:              1,
		IncludesAllRepositories: true,
		IncludesAllDatasets:     true,
		IncludesAllAimodels:     true,
		CanCreateOrgRepo:        true,
		CanCreateOrgDataset:     true,
		CanCreateOrgAimodel:     true,
	}
...
func DeleteOrganization(org *User) (err error) {
	...
	if err = deleteOrg(sess, org); err != nil {
		if IsErrUserOwnRepos(err) {
			return err
		} else if IsErrUserOwnDatasets(err) {
			return err
		} else if IsErrUserOwnAimodels(err) {
			return err
		} else if err != nil {
...
func CanCreateOrgDataset(orgID, uid int64) (bool, error) {
	if owner, err := IsOrganizationOwner(orgID, uid); owner || err != nil {
		return owner, err
	}
	return x.
		Where(builder.Eq{"team.can_create_org_dataset": true}).
		Join("INNER", "team_user", "team_user.team_id = team.id").
		And("team_user.uid = ?", uid).
		And("team_user.org_id = ?", orgID).
		Exist(new(Team))
...
func CanCreateOrgAimodel(orgID, uid int64) (bool, error) {
	if owner, err := IsOrganizationOwner(orgID, uid); owner || err != nil {
		return owner, err
	}
	return x.
		Where(builder.Eq{"team.can_create_org_aimodel": true}).
		Join("INNER", "team_user", "team_user.team_id = team.id").
		And("team_user.uid = ?", uid).
		And("team_user.org_id = ?", orgID).
		Exist(new(Team))
...
// GetOrgsByName 根据昵称查询组织/用户
// userID: 查询用户的ID
// showAll: 是否显示非公开组织
// key: 搜索关键词，逗号分隔多个关键词（AND关系）
func GetOrgsByName(userID int64, showAll bool, key string) ([]*User, error) {
	sess := x.NewSession()
	defer sess.Close()
	...
	// 基础条件：用户关联
	sess.Join("INNER", "`org_user`", "`org_user`.org_id=`user`.id").
		And("`org_user`.uid=?", userID)
	...
	if key != "" {
		...
		for _, keyword := range keywords {
			...
			// 同一个关键词在lower_name或name中匹配（OR关系）
			keywordCond := builder.Or(
				builder.Like{"lower_name", "%" + strings.ToLower(keyword) + "%"},
...
func GetOrgsCanCreateDatasetByUserID(userID int64) ([]*User, error) {
	orgs := make([]*User, 0, 10)
	...
	return orgs, x.Where(builder.In("id", builder.Select("`user`.id").From("`user`").
		...
		Join("INNER", "`team`", "`team`.id = `team_user`.team_id").
		Where(builder.Eq{"`team_user`.uid": userID}).
		And(builder.Eq{"`team`.authorize": AccessModeOwner}.Or(builder.Eq{"`team`.can_create_org_dataset": true})))).
		Desc("`user`.updated_unix").Find(&orgs)
}

func GetOrgsCanCreateAimodelByUserID(userID int64) ([]*User, error) {
	orgs := make([]*User, 0, 10)
	...
	return orgs, x.Where(builder.In("id", builder.Select("`user`.id").From("`user`").
		...
		Join("INNER", "`team`", "`team`.id = `team_user`.team_id").
		Where(builder.Eq{"`team_user`.uid": userID}).
		And(builder.Eq{"`team`.authorize": AccessModeOwner}.Or(builder.Eq{"`team`.can_create_org_aimodel": true})))).
		Desc("`user`.updated_unix").Find(&orgs)
}
...
func (org *User) GetOrgDatasetIds4User(userID int64) ([]string, error) {
	var user *User
	...
	if user == nil || !user.IsRestricted {
		cond = cond.Or(builder.Eq{
			"`dataset_registry`.owner_id":   org.ID,
			"`dataset_registry`.is_private": false,
		})
	}
	if len(teamIDs) > 0 {
		cond = cond.Or(builder.In("team_subject.team_id", teamIDs))
	}
	cond = cond.And(builder.Eq{"`team_subject`.subject_type": DatasetSubject})
	datasetIDs := make([]string, 0)
	return datasetIDs, x.
		Table("dataset_registry").
		Join("INNER", "team_subject", "`team_subject`.subject_id=`dataset_registry`.id").
		Where(cond).
		GroupBy("`dataset_registry`.id").
		Cols("`dataset_registry`.id").
		Find(&datasetIDs)
}

func (org *User) GetOrgAimodelIds4User(userID int64) ([]string, error) {
	var user *User
	teamIDs := make([]int64, 0)
	if userID > 0 {
		u, err := GetUserByID(userID)
	...
	if user == nil || !user.IsRestricted {
		cond = cond.Or(builder.Eq{
			"`ai_model_manage`.owner_id":   org.ID,
			"`ai_model_manage`.is_private": false,
		})
	}
	if len(teamIDs) > 0 {
		cond = cond.Or(builder.In("team_subject.team_id", teamIDs))
	}
	cond = cond.And(builder.Eq{"`team_subject`.subject_type": AimodelSubject})
	aimodelIDs := make([]string, 0)
	return aimodelIDs, x.
		Table("ai_model_manage").
		Join("INNER", "team_subject", "`team_subject`.subject_id::text=`ai_model_manage`.id").
		Where(cond).
		GroupBy("`ai_model_manage`.id").
		Cols("`ai_model_manage`.id").
		Find(&aimodelIDs)
}
