...
type Team struct {
	...
	DatasetAuthorize        AccessMode
	Datasets                []*DatasetRegistry
	NumDatasets             int
	IncludesAllDatasets     bool `xorm:"NOT NULL DEFAULT false"`
	CanCreateOrgDataset     bool `xorm:"NOT NULL DEFAULT false"`
	AimodelAuthorize        AccessMode
	Aimodels                []*AiModelManage
	NumAimodels             int
	IncludesAllAimodels     bool `xorm:"NOT NULL DEFAULT false"`
	CanCreateOrgAimodel     bool `xorm:"NOT NULL DEFAULT false"`
}

type Team4Front struct {
	ID                          int64
	Name                        string
	NumMembers                  int
	NumRepos                    int
	NumDatasets                 int
	IncludesAllDatasets         bool
	AllowedToChangeDatasetTeams bool
	DatasetAuthorize            AccessMode
	OwnerName                   string
	NumAimodels                 int
	IncludesAllAimodels         bool
	AllowedToChangeAimodelTeams bool
	AimodelAuthorize            AccessMode
}
...
func (t *Team) getDatasets(e Engine) error {
	if t.Datasets != nil {
		return nil
	}
	return e.Join("INNER", "team_subject", "dataset_registry.id = team_subject.subject_id").
		Where("team_subject.team_id=? and team_subject.subject_type=?", t.ID, DatasetSubject).
		OrderBy("dataset_registry.name").
		Find(&t.Datasets)
}

func (t *Team) getAimodels(e Engine) error {
	if t.Aimodels != nil {
		return nil
	}
	return e.Join("INNER", "team_subject", "ai_model_manage.id::uuid = team_subject.subject_id").
		Where("team_subject.team_id=? and team_subject.subject_type=?", t.ID, AimodelSubject).
		OrderBy("ai_model_manage.name").
		Find(&t.Aimodels)
}
...
// GetRepositories returns paginated repositories in team of organization.
func (t *Team) GetDatasets(opts *SearchTeamOptions) error {
	if opts.Page == 0 {
		return t.getDatasets(x)
	}

	return t.getDatasets(opts.getPaginatedSession())
}

// GetRepositories returns paginated repositories in team of organization.
func (t *Team) GetAimodels(opts *SearchTeamOptions) error {
	if opts.Page == 0 {
		return t.getAimodels(x)
	}

	return t.getAimodels(opts.getPaginatedSession())
}
...
func (t *Team) hasSubject(e Engine, subjectID string, subjectType SubjectType) bool {
	return hasTeamSubject(e, t.OrgID, t.ID, subjectID, subjectType)
}
...
func (t *Team) HasSubject(subjectID string, subjectType SubjectType) bool {
	return t.hasSubject(x, subjectID, subjectType)
}
...
func (t *Team) addAllDatasets(e Engine) error {
	var orgDatasets []DatasetRegistry
	if err := e.Where("owner_id = ?", t.OrgID).Find(&orgDatasets); err != nil {
		return fmt.Errorf("get org datasets: %v", err)
	}

	for _, dataset := range orgDatasets {
		if !t.hasSubject(e, dataset.ID, DatasetSubject) {
			if err := t.addDataset(e, &dataset); err != nil {
				return fmt.Errorf("addRepository: %v", err)
			}
		}
	}
...
func (t *Team) addDataset(e Engine, dataset *DatasetRegistry) (err error) {
	if err = addTeamSubject(e, t.OrgID, t.ID, dataset.ID, DatasetSubject); err != nil {
		return err
	}

	if _, err = e.Incr("num_datasets").ID(t.ID).Update(new(Team)); err != nil {
		return fmt.Errorf("update team: %v", err)
	}

	t.NumDatasets++

	ctx := dataset.ConvertSubjectAccessContext()
	if err = ctx.recalculateTeamAccesses(e, 0, DatasetSubject); err != nil {
		return fmt.Errorf("recalculateAccesses: %v", err)
	}
...
func (t *Team) addAllAimodels(e Engine) error {
	var orgAimodels []AiModelManage
	if err := e.Where("owner_id = ?", t.OrgID).Find(&orgAimodels); err != nil {
		return fmt.Errorf("get org aimodels: %v", err)
	}

	for _, aimodel := range orgAimodels {
		if !t.hasSubject(e, aimodel.ID, AimodelSubject) {
			if err := t.addAimodel(e, &aimodel); err != nil {
				return fmt.Errorf("addRepository: %v", err)
			}
		}
	}
...
func (t *Team) addAimodel(e Engine, aimodel *AiModelManage) (err error) {
	if err = addTeamSubject(e, t.OrgID, t.ID, aimodel.ID, AimodelSubject); err != nil {
		return err
	}

	if _, err = e.Incr("num_aimodels").ID(t.ID).Update(new(Team)); err != nil {
		return fmt.Errorf("update team: %v", err)
	}

	t.NumAimodels++

	ctx := aimodel.ConvertSubjectAccessContext()
	if err = ctx.recalculateTeamAccesses(e, 0, AimodelSubject); err != nil {
		return fmt.Errorf("recalculateAccesses: %v", err)
	}
...
func addTeamSubject(e Engine, orgID, teamID int64, subjectID string, subjectType SubjectType) error {
	_, err := e.InsertOne(&TeamSubject{
		OrgID:       orgID,
		TeamID:      teamID,
		SubjectID:   subjectID,
...
func (t *Team) AddAllDatasets() (err error) {
	sess := x.NewSession()
	defer sess.Close()
	if err = sess.Begin(); err != nil {
		return err
	...
	if err = t.addAllDatasets(sess); err != nil {
		return err
	}
...
func (t *Team) AddAllAimodels() (err error) {
	sess := x.NewSession()
	defer sess.Close()
	if err = sess.Begin(); err != nil {
		return err
	...
	if err = t.addAllAimodels(sess); err != nil {
		return err
	}
...
func (t *Team) AddDataset(dataset *DatasetRegistry) (err error) {
	if dataset.OwnerID != t.OrgID {
		return errors.New("Dataset does not belong to organization")
	} else if t.HasSubject(dataset.ID, DatasetSubject) {
		return nil
	}
...
func (t *Team) AddAimodel(aimodel *AiModelManage) (err error) {
	if aimodel.OwnerID != t.OrgID {
		return errors.New("Aimodel does not belong to organization")
	} else if t.HasSubject(aimodel.ID, AimodelSubject) {
		return nil
	}
...
func (t *Team) RemoveAllDatasets() (err error) {
	...
	if err = t.removeAllDatasets(sess); err != nil {
		return err
	}
...
func (t *Team) RemoveAllAimodels() (err error) {
	...
	if err = t.removeAllAimodels(sess); err != nil {
		return err
	}
...
func (t *Team) removeAllDatasets(e Engine) (err error) {
	for _, dataset := range t.Datasets {
		ctx := dataset.ConvertSubjectAccessContext()
		if err := ctx.recalculateTeamAccesses(e, t.ID, DatasetSubject); err != nil {
			return err
		}
	}

	if _, err := e.
		Where("team_id=? and subject_type=?", t.ID, DatasetSubject).
		Delete(new(TeamSubject)); err != nil {
		return err
	}

	t.NumDatasets = 0
	if _, err = e.ID(t.ID).Cols("num_datasets").Update(t); err != nil {
		return err
	}
...
func (t *Team) removeAllAimodels(e Engine) (err error) {
	for _, aimodel := range t.Aimodels {
		ctx := aimodel.ConvertSubjectAccessContext()
		if err := ctx.recalculateTeamAccesses(e, t.ID, AimodelSubject); err != nil {
			return err
		}
	}

	if _, err := e.
		Where("team_id=? and subject_type=?", t.ID, AimodelSubject).
		Delete(new(TeamSubject)); err != nil {
		return err
	}

	t.NumAimodels = 0
	if _, err = e.ID(t.ID).Cols("num_aimodels").Update(t); err != nil {
		return err
	}
...
func (t *Team) removeDataset(e Engine, dataset *DatasetRegistry, recalculate bool) (err error) {
	if err = removeTeamSubject(e, t.ID, dataset.ID, DatasetSubject); err != nil {
		return err
	}
	...
	for _, teamUser := range teamUsers {
		has, err := hasSubjectAccess(e, teamUser.UID, ctx)
		if err != nil {
			return err
		} else if has {
			continue
...
func (t *Team) removeAimodel(e Engine, aimodel *AiModelManage, recalculate bool) (err error) {
	if err = removeTeamSubject(e, t.ID, aimodel.ID, AimodelSubject); err != nil {
		return err
	}
...
func (t *Team) RemoveDataset(datasetID string) error {
	if !t.HasSubject(datasetID, DatasetSubject) {
		return nil
	}
	...
	dataset, err := GetDatasetRegistryByID(datasetID)
	if err != nil {
		return err
	}
	...
	if err = t.removeDataset(sess, dataset, true); err != nil {
		return err
	}
...
func (t *Team) RemoveAimodel(aimodelID string) error {
	if !t.HasSubject(aimodelID, AimodelSubject) {
		return nil
	}
	...
	aimodel, err := GetAimodelByID(aimodelID)
	if err != nil {
		return err
	}
	...
	if err = t.removeAimodel(sess, aimodel, true); err != nil {
		return err
	}
...
func (t *Team) RemoveDataset4Old(datasetID string) error {
	if !t.HasSubject(datasetID, DatasetSubject) {
		return nil
	}
...
func NewTeam(t *Team) (err error) {
	...
	if t.IncludesAllAimodels {
		err = t.addAllAimodels(sess)
		if err != nil {
			return fmt.Errorf("addAllAimodels")
		}
	}
...
func UpdateTeam(t *Team, authChanged bool, includeAllChanged bool, datasetAuthChanged bool, includeAllDatasetChanged bool, aimodelAuthChanged bool, includeAllAimodelChanged bool) (err error) {
	if len(t.Name) == 0 {
		return errors.New("empty team name")
	}
	...
	if _, err = sess.ID(t.ID).Cols("name", "lower_name", "description",
		"can_create_org_repo", "can_create_org_dataset", "can_create_org_aimodel",
		"authorize", "includes_all_repositories",
		"dataset_authorize", "includes_all_datasets",
		"aimodel_authorize", "includes_all_aimodels").Update(t); err != nil {
	...
	// Update access for team members if needed.
	if aimodelAuthChanged {
		if err = t.getAimodels(sess); err != nil {
			return fmt.Errorf("getAimodels err: %v", err)
		}

		for _, aimodel := range t.Aimodels {
			subjectCtx := aimodel.ConvertSubjectAccessContext()
			if err = subjectCtx.recalculateTeamAccesses(sess, 0, AimodelSubject); err != nil {
				return fmt.Errorf("recalculateTeamAccesses: %v", err)
			}
		}
	}
	...
	if includeAllAimodelChanged && t.IncludesAllAimodels {
		err = t.addAllAimodels(sess)
		if err != nil {
			return fmt.Errorf("addAllAimodels: %v", err)
		}
	}
...
func DeleteTeam(t *Team) error {
	...
	if err := t.GetDatasets(&SearchTeamOptions{}); err != nil {
		return err
	}
...
func getUserSubjectTeams(e Engine, orgID, userID int64, subjectID string, subjectType SubjectType) (teams []*Team, err error) {
	return teams, e.
		Join("INNER", "team_user", "team_user.team_id = team.id").
		Join("INNER", "team_subject", "team_subject.team_id = team.id").
		Where("team.org_id = ?", orgID).
		And("team_user.uid=?", userID).
		And("team_subject.subject_id=? and team_subject.subject_type=?", subjectID, subjectType).
		Find(&teams)
}
...
func AddTeamMember(team *Team, userID int64) error {
	...
	if err := team.GetDatasets(&SearchTeamOptions{}); err != nil {
		return err
	}
	if err := team.GetAimodels(&SearchTeamOptions{}); err != nil {
		return err
	}
	...
	for _, repo := range team.Datasets {
		ctx := repo.ConvertSubjectAccessContext()
		if err := ctx.recalculateUserAccess(sess, userID); err != nil {
			return err
		}
	}
	for _, repo := range team.Aimodels {
		ctx := repo.ConvertSubjectAccessContext()
		if err := ctx.recalculateUserAccess(sess, userID); err != nil {
			return err
		}
...
func removeTeamMember(e *xorm.Session, team *Team, userID int64) error {
	...
	if err := team.getDatasets(e); err != nil {
		return err
	}

	if err := team.getAimodels(e); err != nil {
		return err
	}
	...
	for _, dataset := range team.Datasets {
		ctx := dataset.ConvertSubjectAccessContext()
		if err := ctx.recalculateUserAccess(e, userID); err != nil {
			return err
		}
	}

	for _, aimodel := range team.Aimodels {
		ctx := aimodel.ConvertSubjectAccessContext()
		if err := ctx.recalculateUserAccess(e, userID); err != nil {
			return err
		}
...
func hasTeamSubject(e Engine, orgID, teamID int64, subjectID string, subjectType SubjectType) bool {
	has, _ := e.
		Where("org_id=?", orgID).
		And("team_id=?", teamID).
...
func HasTeamSubject(orgID, teamID int64, subjectID string, subjectType SubjectType) bool {
	return hasTeamSubject(x, orgID, teamID, subjectID, subjectType)
}
...
func removeTeamSubject(e Engine, teamID int64, subjectID string, subjectType SubjectType) error {
	_, err := e.Delete(&TeamSubject{
		TeamID:      teamID,
		SubjectID:   subjectID,
		SubjectType: int(subjectType),
	})
