...
const (
	...
	tplRoles         base.TplName = "admin/role/index"
	tplBatchAccess   base.TplName = "admin/role/batch"
	tplAccess        base.TplName = "admin/role/access"
...
func prepareUserInfo(ctx *context.Context) *models.User {
	...
	u.OperRole, u.ResourceRole, u.StorageRole = role.QueryUserRole(u.ID)
	ctx.Data["User"] = u
...
func DeleteUser(ctx *context.Context) {
	...
	if err = models.DeleteUser(u); err != nil {
		...
		case models.IsErrUserOwnDatasets(err):
			ctx.Flash.Error(ctx.Tr("admin.users.still_own_dataset"))
			ctx.JSON(200, map[string]interface{}{
				"redirect": setting.AppSubURL + "/admin/users/" + ctx.Params(":userid"),
			})
		case models.IsErrUserOwnAimodels(err):
			ctx.Flash.Error(ctx.Tr("admin.users.still_own_aimodel"))
			ctx.JSON(200, map[string]interface{}{
				"redirect": setting.AppSubURL + "/admin/users/" + ctx.Params(":userid"),
			})
		...
		case models.IsErrUserHasCloudbrainTask(err):
			t, _ := err.(models.ErrUserHasCloudbrainTask)
			ctx.Flash.Error(ctx.Tr("form.still_own_run_task", t.RepoName, t.TaskName))
			ctx.JSON(200, map[string]interface{}{
				"redirect": setting.AppSubURL + "/admin/users/" + ctx.Params(":userid"),
			})
		default:
...
func BatchAccess(ctx *context.Context) {
	ctx.Data["PageIsAdminAccess"] = true
	ctx.HTML(200, tplBatchAccess)
}

func Access(ctx *context.Context) {
	ctx.Data["PageIsAdminAccess"] = true
	ctx.HTML(200, tplAccess)
}
func EditAccess(ctx *context.Context) {
	ctx.Data["PageIsAdminAccess"] = true
	ctx.HTML(200, tplEditAccess)
}

func OrgAccess(ctx *context.Context) {
	ctx.Data["PageIsAdminOrgAccess"] = true
	ctx.HTML(200, tplOrgAccess)
}
func EditOrgAccess(ctx *context.Context) {
	ctx.Data["PageIsAdminOrgAccess"] = true
	ctx.HTML(200, tplEditOrgAccess)
}
