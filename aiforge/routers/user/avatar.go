...
func Avatar(ctx *context.Context) {
	...
	avatarUrl := user.RealSizedAvatarLink(size)
	//log.Info("avatar url=" + avatarUrl)
	if strings.HasSuffix(avatarUrl, "/img/avatar_default.png") {
		ctx.Redirect(user.RealSizedAvatarLink(size))
	} else {
		public.NewStaticHandler(
			avatarUrl,
			setting.AvatarUploadPath,
			&public.Options{
				Prefix:       "avatars",
				SkipLogging:  setting.DisableRouterLog,
				ExpiresAfter: setting.AvatarStaticCacheTime,
			},
			ctx.Context,
		)
	}
