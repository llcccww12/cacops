...
const (
	tplOverview         base.TplName = "user/dashboard/dashboard"
	tplDashboard        base.TplName = "user/dashboard/repos"
	tplDashboard1       base.TplName = "user/dashboard/repos1"
	tplIssues           base.TplName = "user/dashboard/issues"
	tplIssues1          base.TplName = "user/dashboard/issues1"
	tplMilestones       base.TplName = "user/dashboard/milestones"
	tplProfile          base.TplName = "user/profile"
	tplCloudbrains      base.TplName = "user/dashboard/cloudbrains"
	tplCloudbrainCreate base.TplName = "user/dashboard/cloudbraincreate"
	tplCloudbrainDetail base.TplName = "user/dashboard/cloudbraindetail"
)
...
func getDashboardContextUser(ctx *context.Context) *models.User {
	...
	if ctx.User != nil {
		if err := ctx.User.GetOrganizations(&models.SearchOrganizationsOptions{All: true}); err != nil {
			log.Error("GetOrganizations failed: %v", err)
			// GetOrganizations 失败不影响主流程，继续执行
			ctx.Data["Orgs"] = []*models.User{}
		} else {
...
func retrieveFeeds(ctx *context.Context, options models.GetFeedsOptions) {
	...
	for _, act := range actions {
		...
		if act.Dataset != nil {
			datasetOwner, ok := userCache[act.Dataset.OwnerID]
			if !ok {
				...
				userCache[datasetOwner.ID] = datasetOwner
			}
			act.Dataset.Owner = datasetOwner
		}
		if act.Aimodel != nil {
			aimodelOwner, ok := userCache[act.Aimodel.OwnerID]
			if !ok {
				...
				userCache[aimodelOwner.ID] = aimodelOwner
			}
		}
	}
...
func Dashboard1(ctx *context.Context) {
	...
	ctx.HTML(200, tplDashboard1)
}
func OverView(ctx *context.Context) {
	getDashboardContextUser(ctx)
	ctx.HTML(200, tplOverview)
}
...
func Issues(ctx *context.Context) {
	...
	if ctxUser == nil {
		log.Error("ctxUser is nil for path: %s", ctx.Req.URL.Path)
		return
	}
...
func Issues1(ctx *context.Context) {
	isPullList := ctx.Params(":type") == "pulls"
	unitType := models.UnitTypeIssues
	if isPullList {
		if models.UnitTypePullRequests.UnitGlobalDisabled() {
...
func CloudbrainCreate(ctx *context.Context) {
	ctx.HTML(200, tplCloudbrainCreate)
}
func CloudbrainDetail(ctx *context.Context) {
	ctx.HTML(200, tplCloudbrainDetail)
}
