...
func handleUsernameChange(ctx *context.Context, newName string) {
	...
	if len(newName) == 0 || (!ctx.User.IsLocal() && !ctx.User.IsOAuth2()) {
		return
	}
...
func ProfilePost(ctx *context.Context, form auth.UpdateProfileForm) {
	...
	if form.Occupation == 0 {
		ctx.Flash.Error(ctx.Tr("occupation_empty"))
		ctx.Redirect(setting.AppSubURL + "/user/settings")
		return
	}
	...
	ctx.User.Location = util.StripHTML(form.Location)
	ctx.User.Language = util.StripHTML(form.Language)
	ctx.User.Description = util.StripHTML(form.Description)
	ctx.User.Occupation = form.Occupation
...
func UpdateDiffusionAvatarSetting(ctx *context.Context, form auth.AvatarDiffusionUpdateForm, ctxUser *models.User) error {
	return ctxUser.UpdateRandomAvatarUseDiffustionModelByUser(form.ImageBase64)
...
func UpdateAvatarSetting(ctx *context.Context, form auth.AvatarForm, ctxUser *models.User) error {
	...
	if form.Avatar != nil && form.Avatar.Filename != "" {
		...
		if form.Avatar.Size > setting.AvatarMaxFileSize {
			return errors.New(ctx.Tr("settings.uploaded_avatar_is_too_big", setting.AvatarMaxFileSize/1048576))
		}
...
func AvatarDiffusionGeneratePost(ctx *context.Context, form auth.AvatarDiffusionForm) {

	prompt := ""
	if len(setting.AIAvartarPromoteConfig.AnimalPrompt) == 0 {
		setting.GetDiffusionAvatarConfig()
	}
	if form.GenerateType == 0 { //animal
		prompt = setting.AIAvartarPromoteConfig.AnimalPrompt[rand.Intn(len(setting.AIAvartarPromoteConfig.AnimalPrompt))]
	} else if form.GenerateType == 1 { //landscape
		prompt = setting.AIAvartarPromoteConfig.LandscapePrompt[rand.Intn(len(setting.AIAvartarPromoteConfig.LandscapePrompt))]
	} else if form.GenerateType == 2 { //human
		prompt = setting.AIAvartarPromoteConfig.HumanPrompt[rand.Intn(len(setting.AIAvartarPromoteConfig.HumanPrompt))]
	} else if form.GenerateType == 3 { //female
		prompt = setting.AIAvartarPromoteConfig.FemalePrompt[rand.Intn(len(setting.AIAvartarPromoteConfig.FemalePrompt))]
	} else { //male
		prompt = setting.AIAvartarPromoteConfig.MalePrompt[rand.Intn(len(setting.AIAvartarPromoteConfig.MalePrompt))]

	}

	imageBase64, err := avatar.RandomDiffusionBase64Image(setting.AIAVartarSize, prompt)
	if err != nil {
		ctx.JSON(http.StatusOK, models.BaseErrorMessageApi(err.Error()))
		return
	}
	ctx.JSON(http.StatusOK, models.BaseMessageApi{Code: 0, Message: imageBase64})

}

func AvatarDiffusionUpdatePost(ctx *context.Context, form auth.AvatarDiffusionUpdateForm) {
	if err := UpdateDiffusionAvatarSetting(ctx, form, ctx.User); err != nil {
		ctx.Flash.Error(err.Error())
	} else {
		notification.NotifyChangeUserAvatar(ctx.User, auth.AvatarForm{})
		ctx.Flash.Success(ctx.Tr("settings.update_avatar_success"))
	}
