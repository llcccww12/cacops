...
func HookPostReceive(ctx *macaron.Context, opts private.HookOptions) {
	...
	for i := range opts.OldCommitIDs {
		...
		if strings.HasPrefix(refFullName, git.BranchPrefix) || strings.HasPrefix(refFullName, git.TagPrefix) {
			...
			//删除缓存
			go deleteCommitRelatedCache(repo, option.BranchName())
...
func deleteCommitRelatedCache(repo *models.Repository, currentBranchName string) {
	aiTemplateKey := redis_key.RepoAITemplateCache(repo.ID, currentBranchName)
	redis_client.Del(aiTemplateKey)
