...
func CreateFork(ctx *context.APIContext, form api.CreateForkOption) {
	...
	fork, err := repo_service.ForkRepository(ctx.User, forker, repo, repoName, template.HTMLEscapeString(repo.Description), alias)
	if err != nil {
		if models.IsErrRepoAlreadyExist(err) {
			ctx.Error(http.StatusInternalServerError, "RepoAlreadyExist", ctx.Tr("repo.RepoAlreadyExist"))
		} else if models.IsErrForkAlreadyExist(err) {
			ctx.Error(http.StatusInternalServerError, "ForkAlreadyExist", ctx.Tr("repo.ForkAlreadyExist", err.(models.ErrForkAlreadyExist).ForkName))
		} else {
