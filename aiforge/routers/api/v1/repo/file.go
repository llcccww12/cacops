...
func CommitFiles(ctx *context.APIContext, commitMultiFileOptions api.CommitMultiFileOptions) {
	...
	// 当前分支不存在或被删除了
	if !git.IsBranchExist(ctx.Repo.Repository.RepoPath(), currentBranch) {
		ctx.Error(http.StatusBadRequest, "Error", "当前分支不存在或已删除")
		return
	}
	// 提交的分支不存在，checkout一个新分支出来
	if !git.IsBranchExist(ctx.Repo.Repository.RepoPath(), commitBranch) {
		// 不存在，从当前分支checkout一个新分支
	...
	for _, apiOpt := range apiOpts {
		...
		case "add":
			if apiOpt.BranchName == "" {
				apiOpt.BranchName = commitBranch
			}
