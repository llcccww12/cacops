...
func ListIssues(ctx *context.APIContext) {
	...
	asDict := ctx.QueryBool("asdict")
	if asDict {
		data := map[string]interface{}{
			"Total":    ctx.Repo.Repository.NumIssues,
			"Page":     listOptions.Page,
			"PageSize": listOptions.PageSize,
			"Issues":   convert.ToAPIIssueList(issues),
		}
		ctx.JSON(http.StatusOK, response.OuterSuccessWithData(data))
		return
	}
