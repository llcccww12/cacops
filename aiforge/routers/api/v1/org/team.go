...
func EditTeam(ctx *context.APIContext, form api.EditTeamOption) {
	...
	if !team.IsOwnerTeam() && len(form.Permission) != 0 {
		...
		if team.DatasetAuthorize != datasetAuth {
			isDatasetAuthChanged = true
			team.DatasetAuthorize = datasetAuth
		}

		aimodelAuth := models.ParseAccessMode(form.AimodelPermission)
		if team.AimodelAuthorize != aimodelAuth {
			isAimodelAuthChanged = true
			team.AimodelAuthorize = aimodelAuth
		}
		...
		if form.IncludesAllDatasets != nil {
			isDatasetIncludeAllChanged = true
			team.IncludesAllDatasets = *form.IncludesAllDatasets
		}

		if form.IncludesAllAimodels != nil {
			isAimodelIncludeAllChanged = true
			team.IncludesAllAimodels = *form.IncludesAllAimodels
		}
	}
	...
	if err := models.UpdateTeam(team, isAuthChanged, isIncludeAllChanged, isDatasetAuthChanged, isDatasetIncludeAllChanged, isAimodelAuthChanged, isAimodelIncludeAllChanged); err != nil {
