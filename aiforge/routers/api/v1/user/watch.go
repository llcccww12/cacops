...
// watching
func getWatchedRepos(user *models.User, private bool, listOptions models.ListOptions, keyword string) ([]*api.Repository, int64, error) {
	count, err := models.GetWatchedReposCount(user.ID, private, keyword)
	if err != nil {
		return nil, 0, err
	}

	watchedRepos, err := models.GetWatchedRepos(user.ID, private, listOptions, keyword)
	if err != nil {
		return nil, 0, err
	}
...
func GetWatchedRepos(ctx *context.APIContext) {
	...
	repos, count, err := getWatchedRepos(user, private, listOptions, "")
	if err != nil {
...
func GetMyWatchedRepos(ctx *context.APIContext) {
	...
	//   "200":
	//     "$ref": "#/responses/RepositoryList"
	keword := strings.ReplaceAll(strings.Trim(ctx.Query("q"), " "), "'", "")

	listOptions := utils.GetListOptions(ctx)
	repos, count, err := getWatchedRepos(ctx.User, true, listOptions, keword)
	if err != nil {
		ctx.Error(http.StatusInternalServerError, "getWatchedRepos", err)
		return
	}
