...
func DeleteUser(ctx *context.APIContext) {
	...
	if err := models.DeleteUser(u); err != nil {
		if models.IsErrUserOwnRepos(err) ||
			models.IsErrUserHasOrgs(err) || models.IsErrUserOwnDatasets(err) || models.IsErrUserOwnAimodels(err) {
			ctx.Error(http.StatusUnprocessableEntity, "", err)
		} else {
...
func ChangeDefaultUserPortrait(ctx *context.APIContext, userOption api.UserNames) {
	go changeDefaultUserPortrait(userOption)
	ctx.JSON(http.StatusOK, models.BaseOKMessage)
}

func changeDefaultUserPortrait(userOption api.UserNames) {
	for _, name := range userOption.Names {
		user, err := models.GetUserByName(name)

		if err != nil {
			log.Error("UpdateRandomAvatarUseDiffustionModel failed GetUserByID [%s]: %v", name, err)
			continue
		}
		if !user.UseCustomAvatar {
			err = user.UpdateRandomAvatarUseDiffustionModel()
			if err != nil {
				log.Error("UpdateRandomAvatarUseDiffustionModel failed[%s]: %v", name, err)
