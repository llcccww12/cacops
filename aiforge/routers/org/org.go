...
const (
	...
	tplOrgModels   base.TplName = "org/models"
	tplOrgDatasets base.TplName = "org/datasets"
	tplOrgStorages base.TplName = "org/storages"
)
...
func CreatePost(ctx *context.Context, form auth.CreateOrgForm) {
	...
	org := &models.User{
		...
		DatasetAdminChangeTeamAccess: form.DatasetAdminChangeTeamAccess,
		AimodelAdminChangeTeamAccess: form.AimodelAdminChangeTeamAccess,
	}
	...
	// 创建组织后跳转到组织首页
	ctx.Redirect(setting.AppSubURL + "/" + form.OrgName)
}

// models page
func Models(ctx *context.Context) {
	ctx.HTML(200, tplOrgModels)
}

// models page
func Datasets(ctx *context.Context) {
	ctx.HTML(200, tplOrgDatasets)
}

// models page
func Storages(ctx *context.Context) {
	ctx.HTML(200, tplOrgStorages)
}
