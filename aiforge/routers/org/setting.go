...
func Settings(ctx *context.Context) {
	...
	ctx.Data["DatasetAdminChangeTeamAccess"] = ctx.Org.Organization.DatasetAdminChangeTeamAccess
	ctx.Data["AimodelAdminChangeTeamAccess"] = ctx.Org.Organization.AimodelAdminChangeTeamAccess
...
func SettingsPost(ctx *context.Context, form auth.UpdateOrgSettingForm) {
	...
	org.Description = util.StripHTML(form.Description)
	org.Website = form.Website
	org.Location = util.StripHTML(form.Location)
	org.Visibility = form.Visibility
	org.RepoAdminChangeTeamAccess = form.RepoAdminChangeTeamAccess
	org.DatasetAdminChangeTeamAccess = form.DatasetAdminChangeTeamAccess
	org.AimodelAdminChangeTeamAccess = form.AimodelAdminChangeTeamAccess
...
func SettingsDelete(ctx *context.Context) {
	...
	if ctx.Req.Method == "POST" {
		...
		if err := models.DeleteOrganization(org); err != nil {
			...
			} else if models.IsErrUserOwnDatasets(err) {
				ctx.Flash.Error(ctx.Tr("form.org_still_own_dataset"))
				ctx.Redirect(ctx.Org.OrgLink + "/settings/delete")
			} else if models.IsErrUserOwnAimodels(err) {
				ctx.Flash.Error(ctx.Tr("form.org_still_own_aimodel"))
				ctx.Redirect(ctx.Org.OrgLink + "/settings/delete")
			} else {
